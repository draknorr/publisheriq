...
/**
 * Command Palette Home View for Companies
 *
 * Default view with:
 * - Search input with filter syntax parsing
 * - Quick presets (4 company presets)
 * - Quick filters (7 stackable filters)
 * - Genre chips
...
import type { FilterOption, PresetId, QuickFilterId } from '../../lib/companies-types';
...
export function CommandPaletteHome({
  ...
  const visiblePresets = presetsExpanded ? presets : presets.slice(0, 4);
  ...
  const quickFilterGroups = useMemo(() => [
    { label: 'Size', ids: ['major', 'prolific', 'active'] },
    { label: 'Revenue', ids: ['revenue1m', 'revenue10m', 'owners100k'] },
    { label: 'Growth', ids: ['trending'] },
  ], []);
  ...
  const handleApplyParsedFilter = () => {
    ...
    if (filter.type === 'quick_filter') {
      ...
      onApplyPreset(p.id as PresetId);
      close();
    } else {
      applyParsedFilter();
    }
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search filters, or type revenue > 1M..."
