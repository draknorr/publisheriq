      ...
      <div className="animate-in fade-in duration-300">
        {activeTab === 'health' && (
          <div className="space-y-8">
            ...
            <RunningJobsCard jobs={data.runningJobs} />
            ...
            {/* Recent Jobs */}
            <ExpandableJobsTable jobs={data.recentJobs as SyncJobDetail[]} />
          </div>
        )}
              ...
              <div className="overflow-hidden rounded-lg border border-gray-800">
                <table className="w-full">
                  <thead className="bg-gray-900">
                    <tr>
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-400">Job Type</th>
                      ...
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-400">GitHub Run</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-800">
                    {filteredJobs.map((job) => (
                      <tr key={job.id} className="bg-gray-900/50 hover:bg-gray-900">
                        <td className="px-4 py-3">
                          ...
                          {job.batch_size && (
                            <span className="ml-2 text-xs text-gray-500">(batch: {job.batch_size})</span>
                          )}
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <span className="text-sm text-green-400">{job.items_succeeded}</span>
                            <span className="text-gray-600">/</span>
                            <span className="text-sm text-gray-300">{job.items_processed}</span>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {filteredJobs
                    ...
                    .map((job) => (
                      <div key={job.id} className="rounded-lg border border-red-900/50 bg-red-950/20 p-4">
                        <div className="flex items-center justify-between">
                          <span className="font-medium text-red-400">{job.job_type}</span>
                          <span className="text-sm text-gray-500">{new Date(job.started_at).toLocaleString()}</span>
                        </div>
