-- Migration: Add platform filtering support to companies page
-- Date: 2026-01-14
--
-- This migration:
--   1. Adds platform_array TEXT[] to publisher_metrics and developer_metrics
--   2. Creates GIN indexes for fast platform containment queries
--   3. Updates get_companies_with_filters to implement platform filtering
--   4. Updates get_companies_aggregate_stats with platform support
--
-- Platform data comes from apps.platforms (comma-separated string like "windows,mac,linux")
-- Converts to array for efficient filtering using && (any) and @> (all) operators

-- ============================================================================
-- STEP 1: Recreate publisher_metrics with platform_array
-- ============================================================================
...
-- NEW: Pre-compute platform array per publisher
...
GROUP BY pa.publisher_id, p.name, p.game_count, pd.unique_developers,
         pg.genre_ids, pt.tag_ids, pc.category_ids, psd.best_steam_deck_category, pp.platform_array;
...
-- NEW: GIN index for platform array
...
COMMENT ON MATERIALIZED VIEW publisher_metrics IS
  'Pre-computed aggregate metrics for publishers. Includes genre_ids, tag_ids, category_ids, platform_array for fast filtering. Refresh with: REFRESH MATERIALIZED VIEW CONCURRENTLY publisher_metrics';


-- ============================================================================
-- STEP 2: Recreate developer_metrics with platform_array
-- ============================================================================
...
-- NEW: Pre-compute platform array per developer
...
GROUP BY da.developer_id, d.name, d.game_count,
         dg.genre_ids, dt.tag_ids, dc.category_ids, dsd.best_steam_deck_category, dp.platform_array;
...
COMMENT ON MATERIALIZED VIEW developer_metrics IS
  'Pre-computed aggregate metrics for developers. Includes genre_ids, tag_ids, category_ids, platform_array for fast filtering. Refresh with: REFRESH MATERIALIZED VIEW CONCURRENTLY developer_metrics';
...
-- ============================================================================
-- STEP 3: Update get_companies_with_filters with platform filtering
-- ============================================================================
...
BEGIN
  ...
  -- Fast path: No growth filters, no relationship filters
  ...
  -- Slow path: Growth/relationship filters (with proper column aliases)
  ELSE
    RETURN QUERY
    WITH base_companies AS (
      SELECT
...
'Unified RPC for /companies page. All three filter bugs fixed:
- BUG #1: Slow path column aliases
- BUG #2: Platform filtering implemented
- BUG #3: Max filters and genre_mode support';


-- ============================================================================
-- STEP 4: Update get_companies_aggregate_stats with platform filtering
-- ============================================================================
...
'Returns aggregate stats for filtered companies. All filter bugs fixed including platform filtering.';
