-- Migration: Add all-history exact-title filtering to Change Feed activity/news RPCs
-- Purpose:
--   1. Allow exact-title /changes filters to page through all stored history
--   2. Keep global feed reads capped to recent ranges unless app IDs are present
--   3. Remove intermediate app-id-only overloads before adding p_all_history signatures

DROP FUNCTION IF EXISTS get_change_feed_activity(
  INTEGER,
  TEXT,
  TEXT,
  TEXT[],
...
DROP FUNCTION IF EXISTS get_change_feed_news(
  INTEGER,
  TEXT[],
  INTEGER[],
  TEXT,
...
-- The activity RPC calls the final news RPC signature while its SQL body is
-- validated. Define a no-row bootstrap first, then replace it with the real
-- optimized implementation below in the same transaction.
...
AS $$
  WITH request_config AS (
    SELECT
      ...
      COALESCE(p_all_history, FALSE) AS requested_all_history,
  ...
  request_flags AS (
    SELECT
      ...
      (
        rf.requested_all_history
        AND rf.requested_appids IS NOT NULL
        AND CARDINALITY(rf.requested_appids) > 0
      ) AS allow_all_history
  ...
  classified_change_events AS (
    ...
    CROSS JOIN LATERAL (
      SELECT e.*
      FROM UNNEST(rf.requested_appids) AS requested_appids(appid)
      JOIN app_change_events e ON e.appid = requested_appids.appid
      WHERE rf.allow_change_rows
        AND rf.requested_appids IS NOT NULL
        AND (
          rf.allow_all_history
          OR e.occurred_at >= NOW() - make_interval(days => LEAST(rf.days, 30))
        )
      ...
      WHERE rf.allow_change_rows
        AND rf.requested_appids IS NULL
        AND e.occurred_at >= NOW() - make_interval(days => LEAST(rf.days, 30))
    ) e
...
AS $$
  ...
  request_flags AS (
    SELECT
      rc.*,
      (
        COALESCE(p_all_history, FALSE)
        AND rc.requested_appids IS NOT NULL
        AND CARDINALITY(rc.requested_appids) > 0
      ) AS allow_all_history
    FROM request_config rc
  ),
  base_news AS (
    ...
    FROM request_flags rf
    JOIN LATERAL UNNEST(rf.requested_appids) AS requested_appids(appid) ON TRUE
    JOIN steam_news_items n ON n.appid = requested_appids.appid
    JOIN apps a ON a.appid = n.appid
    WHERE rf.requested_appids IS NOT NULL
      AND (
        rf.allow_all_history
        OR COALESCE(n.published_at, n.first_seen_at) >= NOW() - make_interval(days => rf.days)
      )
      AND (
        rf.requested_app_types IS NULL
        OR a.type::TEXT = ANY (rf.requested_app_types)
    ...
    JOIN apps a ON a.appid = n.appid
    CROSS JOIN request_flags rf
    WHERE rf.requested_appids IS NULL
      AND COALESCE(n.published_at, n.first_seen_at) >= NOW() - make_interval(days => rf.days)
      AND (
        rf.requested_app_types IS NULL
        OR a.type::TEXT = ANY (rf.requested_app_types)
      )
  ...
  filtered_news AS (
    ...
    WHERE (
        rf.requested_search IS NULL
        OR bn.app_name ILIKE '%' || rf.requested_search || '%'
        OR COALESCE(lv.title, '') ILIKE '%' || rf.requested_search || '%'
      )
  )
  SELECT
    fn.gid,
...
COMMENT ON FUNCTION get_change_feed_activity(INTEGER, TEXT, TEXT, TEXT[], INTEGER[], BOOLEAN, TEXT, TEXT[], TEXT, DOUBLE PRECISION, TIMESTAMPTZ, TEXT, INTEGER) IS
  'Returns a unified activity-card stream for /changes with optional app-id all-history filtering and keyset pagination.';

COMMENT ON FUNCTION get_change_feed_news(INTEGER, TEXT[], INTEGER[], BOOLEAN, TEXT, TIMESTAMPTZ, TEXT, INTEGER) IS
  'Returns one row per Steam news item gid for /changes with optional app-id all-history filtering.';

REVOKE EXECUTE ON FUNCTION get_change_feed_activity(INTEGER, TEXT, TEXT, TEXT[], INTEGER[], BOOLEAN, TEXT, TEXT[], TEXT, DOUBLE PRECISION, TIMESTAMPTZ, TEXT, INTEGER) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION get_change_feed_activity(INTEGER, TEXT, TEXT, TEXT[], INTEGER[], BOOLEAN, TEXT, TEXT[], TEXT, DOUBLE PRECISION, TIMESTAMPTZ, TEXT, INTEGER) FROM anon;
GRANT EXECUTE ON FUNCTION get_change_feed_activity(INTEGER, TEXT, TEXT, TEXT[], INTEGER[], BOOLEAN, TEXT, TEXT[], TEXT, DOUBLE PRECISION, TIMESTAMPTZ, TEXT, INTEGER) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION get_change_feed_news(INTEGER, TEXT[], INTEGER[], BOOLEAN, TEXT, TIMESTAMPTZ, TEXT, INTEGER) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION get_change_feed_news(INTEGER, TEXT[], INTEGER[], BOOLEAN, TEXT, TIMESTAMPTZ, TEXT, INTEGER) FROM anon;
GRANT EXECUTE ON FUNCTION get_change_feed_news(INTEGER, TEXT[], INTEGER[], BOOLEAN, TEXT, TIMESTAMPTZ, TEXT, INTEGER) TO authenticated, service_role;
