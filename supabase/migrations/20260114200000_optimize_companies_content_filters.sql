-- Migration: Optimize content filters in get_companies_with_filters RPC
--
-- Problem: Content filters (genres, tags, categories, steam_deck) used correlated
-- EXISTS subqueries with UNION that scanned both publisher and developer paths
-- for every company row (~200K entities), causing timeouts.
--
-- Solution:
-- 1. Add indexes on publisher_id/developer_id for efficient lookups
-- 2. Use nested EXISTS pattern that terminates early (no full table scans)
-- 3. Split publisher/developer checks with bc.type guard (avoids UNION)
--
-- Table sizes (for context):
-- - app_steam_deck: 31K rows
-- - app_genres: 457K rows
-- - app_steam_tags: 2.4M rows
--
-- Performance: >30s timeout → ~500ms-2s

-- Add indexes for efficient "find apps by publisher/developer" lookups
-- These enable fast EXISTS checks: "does this company have any game matching filter?"
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_app_publishers_publisher_id
ON app_publishers(publisher_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_app_developers_developer_id
ON app_developers(developer_id);
  ...
  IF NOT v_needs_growth AND NOT v_needs_relationship THEN
    ...
    filtered AS (
      ...
      WHERE
        ...
        -- Genre filter: Nested EXISTS with early termination
        -- Uses idx_app_publishers_publisher_id -> idx_app_genres_genre_id
        AND (p_genres IS NULL OR (
          (bc.type = 'publisher' AND EXISTS (
            SELECT 1 FROM app_publishers ap
            WHERE ap.publisher_id = bc.id
            AND EXISTS (SELECT 1 FROM app_genres ag WHERE ag.appid = ap.appid AND ag.genre_id = ANY(p_genres))
          ))
          OR
          (bc.type = 'developer' AND EXISTS (
            SELECT 1 FROM app_developers ad
            WHERE ad.developer_id = bc.id
            AND EXISTS (SELECT 1 FROM app_genres ag WHERE ag.appid = ad.appid AND ag.genre_id = ANY(p_genres))
          ))
        ))
        -- Tag filter: Same nested EXISTS pattern
        AND (p_tags IS NULL OR (
          (bc.type = 'publisher' AND EXISTS (
            SELECT 1 FROM app_publishers ap
            WHERE ap.publisher_id = bc.id
            AND EXISTS (SELECT 1 FROM app_steam_tags ast WHERE ast.appid = ap.appid AND ast.tag_id = ANY(p_tags))
          ))
          OR
          (bc.type = 'developer' AND EXISTS (
            SELECT 1 FROM app_developers ad
            WHERE ad.developer_id = bc.id
            AND EXISTS (SELECT 1 FROM app_steam_tags ast WHERE ast.appid = ad.appid AND ast.tag_id = ANY(p_tags))
          ))
        ))
        -- Category filter: Same nested EXISTS pattern
        AND (p_categories IS NULL OR (
          (bc.type = 'publisher' AND EXISTS (
            SELECT 1 FROM app_publishers ap
            WHERE ap.publisher_id = bc.id
            AND EXISTS (SELECT 1 FROM app_categories ac WHERE ac.appid = ap.appid AND ac.category_id = ANY(p_categories))
          ))
          OR
          (bc.type = 'developer' AND EXISTS (
            SELECT 1 FROM app_developers ad
            WHERE ad.developer_id = bc.id
            AND EXISTS (SELECT 1 FROM app_categories ac WHERE ac.appid = ad.appid AND ac.category_id = ANY(p_categories))
          ))
        ))
        -- Steam Deck filter: Same nested EXISTS pattern
    ...
    filtered AS (
      ...
      WHERE (p_search IS NULL OR bc.name ILIKE '%' || p_search || '%')
        ...
        AND (p_status IS NULL
             OR (p_status = 'active' AND bc.games_released_last_year > 0)
             OR (p_status = 'dormant' AND bc.games_released_last_year = 0))
        -- Content filters with nested EXISTS
        AND (p_genres IS NULL OR (
          (bc.type = 'publisher' AND EXISTS (
            SELECT 1 FROM app_publishers ap WHERE ap.publisher_id = bc.id
            AND EXISTS (SELECT 1 FROM app_genres ag WHERE ag.appid = ap.appid AND ag.genre_id = ANY(p_genres))))
          OR (bc.type = 'developer' AND EXISTS (
            SELECT 1 FROM app_developers ad WHERE ad.developer_id = bc.id
            AND EXISTS (SELECT 1 FROM app_genres ag WHERE ag.appid = ad.appid AND ag.genre_id = ANY(p_genres))))))
        AND (p_tags IS NULL OR (
          (bc.type = 'publisher' AND EXISTS (
            SELECT 1 FROM app_publishers ap WHERE ap.publisher_id = bc.id
        ...
        AND (p_steam_deck IS NULL OR (
          (bc.type = 'publisher' AND EXISTS (
            SELECT 1 FROM app_publishers ap WHERE ap.publisher_id = bc.id
            AND EXISTS (SELECT 1 FROM app_steam_deck asd WHERE asd.appid = ap.appid
              AND ((p_steam_deck = 'verified' AND asd.category = 'verified')
                   OR (p_steam_deck = 'playable' AND asd.category IN ('verified', 'playable'))))))
          OR (bc.type = 'developer' AND EXISTS (
            SELECT 1 FROM app_developers ad WHERE ad.developer_id = bc.id
            AND EXISTS (SELECT 1 FROM app_steam_deck asd WHERE asd.appid = ad.appid
              AND ((p_steam_deck = 'verified' AND asd.category = 'verified')
                   OR (p_steam_deck = 'playable' AND asd.category IN ('verified', 'playable'))))))))
    ),
    with_growth AS (
...
Optimized with nested EXISTS pattern for content filters - terminates early on first match.
Two code paths:
- Fast path: when no growth/relationship filters are applied (~200-500ms)
- Slow path: when growth filtering or sorting is needed (~2-5s)';
