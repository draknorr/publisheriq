-- Migration: Pre-compute growth metrics in materialized views
-- Date: 2026-01-14
--
-- PROBLEM: The slow path in get_companies_with_filters times out because it computes
--          growth metrics via expensive correlated subqueries for each company.
--
-- SOLUTION: Pre-compute ccu_growth_7d_percent and ccu_growth_30d_percent in the
--           materialized views. This eliminates the slow path entirely.
--
-- RISK: LOW - purely additive change, all existing columns preserved

-- ============================================================================
-- STEP 1: Recreate publisher_metrics with growth columns
-- ============================================================================
...
-- NEW: Pre-compute CCU growth from ccu_snapshots
publisher_growth AS (
  SELECT
    ap.publisher_id,
    -- 7-day growth: compare last 7 days vs prior 7 days
    AVG(cs.player_count) FILTER (WHERE cs.snapshot_time > NOW() - INTERVAL '7 days') AS current_7d_avg,
    AVG(cs.player_count) FILTER (
      WHERE cs.snapshot_time > NOW() - INTERVAL '14 days'
        AND cs.snapshot_time <= NOW() - INTERVAL '7 days'
    ) AS prior_7d_avg,
    -- 30-day growth: compare last 7 days vs full 30-day baseline
    AVG(cs.player_count) FILTER (WHERE cs.snapshot_time > NOW() - INTERVAL '30 days') AS baseline_30d_avg
  FROM app_publishers ap
  JOIN apps a ON a.appid = ap.appid AND a.type = 'game' AND a.is_delisted = FALSE
  LEFT JOIN ccu_snapshots cs ON cs.appid = ap.appid AND cs.snapshot_time > NOW() - INTERVAL '30 days'
  GROUP BY ap.publisher_id
)
SELECT
  pa.publisher_id,
  ...
  pp.platform_array,
  -- NEW: Pre-computed growth metrics
  CASE
    WHEN pgr.prior_7d_avg > 0
    THEN ROUND(((pgr.current_7d_avg - pgr.prior_7d_avg) / pgr.prior_7d_avg) * 100, 2)
    ELSE NULL
  END::DECIMAL AS ccu_growth_7d_percent,
  CASE
    WHEN pgr.baseline_30d_avg > 0
    THEN ROUND(((pgr.current_7d_avg - pgr.baseline_30d_avg) / pgr.baseline_30d_avg) * 100, 2)
    ELSE NULL
...
GROUP BY pa.publisher_id, p.name, p.game_count, pd.unique_developers,
         ...
         pp.platform_array, pgr.current_7d_avg, pgr.prior_7d_avg, pgr.baseline_30d_avg;

-- Indexes for publisher_metrics (same as before, plus growth)
...
-- NEW: Growth indexes for sorting and filtering
...
COMMENT ON MATERIALIZED VIEW publisher_metrics IS
  'Pre-computed aggregate metrics for publishers with CCU growth. Refresh with: REFRESH MATERIALIZED VIEW CONCURRENTLY publisher_metrics';


-- ============================================================================
-- STEP 2: Recreate developer_metrics with growth columns
-- ============================================================================
...
developer_growth AS (
  ...
  FROM app_developers ad
  JOIN apps a ON a.appid = ad.appid AND a.type = 'game' AND a.is_delisted = FALSE
  LEFT JOIN ccu_snapshots cs ON cs.appid = ad.appid AND cs.snapshot_time > NOW() - INTERVAL '30 days'
  GROUP BY ad.developer_id
)
SELECT
  da.developer_id,
  ...
  -- NEW: Pre-computed growth metrics
  CASE
    WHEN dgr.prior_7d_avg > 0
    THEN ROUND(((dgr.current_7d_avg - dgr.prior_7d_avg) / dgr.prior_7d_avg) * 100, 2)
    ELSE NULL
  END::DECIMAL AS ccu_growth_7d_percent,
  CASE
    WHEN dgr.baseline_30d_avg > 0
    THEN ROUND(((dgr.current_7d_avg - dgr.baseline_30d_avg) / dgr.baseline_30d_avg) * 100, 2)
    ELSE NULL
  END::DECIMAL AS ccu_growth_30d_percent,
  NOW() AS computed_at
...
GROUP BY da.developer_id, d.name, d.game_count,
         ...
         dp.platform_array, dgr.current_7d_avg, dgr.prior_7d_avg, dgr.baseline_30d_avg;

-- Indexes for developer_metrics (same as before, plus growth)
...
COMMENT ON MATERIALIZED VIEW developer_metrics IS
  'Pre-computed aggregate metrics for developers with CCU growth. Refresh with: REFRESH MATERIALIZED VIEW CONCURRENTLY developer_metrics';
...
-- ============================================================================
-- STEP 3: Update get_companies_with_filters to use pre-computed growth
-- ============================================================================
-- The key change: eliminate the slow path by reading growth from materialized views
...
BEGIN
  ...
  -- Fast path: Use pre-computed growth from materialized views
  -- Only falls back to slow path for relationship filters (which require additional JOINs)
  IF NOT v_needs_relationship THEN
    ...
    WITH base_companies AS (
      SELECT
        ...
        pm.platform_array,
        -- NEW: Pre-computed growth from materialized view
    ...
    filtered AS (
      ...
      WHERE
        ...
        -- NEW: Growth filters using pre-computed values
  ...
  -- Slow path: Only for relationship filters (which require additional JOINs)
  ELSE
    RETURN QUERY
    WITH base_companies AS (
      SELECT
    ...
    filtered AS (
      ...
      WHERE (p_search IS NULL OR bc.name ILIKE '%' || p_search || '%')
        ...
        -- Growth filters using pre-computed values
        AND (p_min_growth_7d IS NULL OR bc.ccu_growth_7d_percent >= p_min_growth_7d)
        AND (p_max_growth_7d IS NULL OR bc.ccu_growth_7d_percent <= p_max_growth_7d)
        AND (p_min_growth_30d IS NULL OR bc.ccu_growth_30d_percent >= p_min_growth_30d)
        AND (p_max_growth_30d IS NULL OR bc.ccu_growth_30d_percent <= p_max_growth_30d)
    ...
    SELECT ff.id, ff.name, ff.type, ff.game_count, ff.total_owners, ff.total_ccu, ff.estimated_weekly_hours,
      ...
      NULL::DECIMAL AS review_velocity_7d, NULL::DECIMAL AS review_velocity_30d,
...
'Unified RPC for /companies page with pre-computed growth metrics. Growth filters now use fast path.';


-- ============================================================================
-- STEP 4: Update get_companies_aggregate_stats with growth filter support
-- ============================================================================

DROP FUNCTION IF EXISTS get_companies_aggregate_stats(
  TEXT, TEXT, INT, INT, BIGINT, BIGINT, INT, INT, BIGINT, BIGINT, BIGINT, BIGINT, INT, INT, INT, INT,
  TEXT, INT[], TEXT, INT[], INT[], TEXT, TEXT[], TEXT
);
...
BEGIN
  ...
  WITH base AS (
    SELECT pm.game_count, pm.total_owners, pm.revenue_estimate_cents, pm.avg_review_score, pm.total_ccu,
           ...
           pm.genre_ids, pm.tag_ids, pm.category_ids, pm.best_steam_deck_category, pm.platform_array,
           pm.ccu_growth_7d_percent, pm.ccu_growth_30d_percent
    ...
    SELECT dm.game_count, dm.total_owners, dm.revenue_estimate_cents, dm.avg_review_score, dm.total_ccu,
           ...
           dm.genre_ids, dm.tag_ids, dm.category_ids, dm.best_steam_deck_category, dm.platform_array,
           dm.ccu_growth_7d_percent, dm.ccu_growth_30d_percent
  ...
  filtered AS (
    ...
    WHERE (p_search IS NULL OR b.name ILIKE '%' || p_search || '%')
      ...
      AND (p_platforms IS NULL OR (
        CASE p_platform_mode
          WHEN 'all' THEN b.platform_array @> p_platforms
          ELSE b.platform_array && p_platforms
        END
      ))
      -- NEW: Growth filters
      AND (p_min_growth_7d IS NULL OR b.ccu_growth_7d_percent >= p_min_growth_7d)
      AND (p_max_growth_7d IS NULL OR b.ccu_growth_7d_percent <= p_max_growth_7d)
      AND (p_min_growth_30d IS NULL OR b.ccu_growth_30d_percent >= p_min_growth_30d)
      AND (p_max_growth_30d IS NULL OR b.ccu_growth_30d_percent <= p_max_growth_30d)
  )
...
COMMENT ON FUNCTION get_companies_aggregate_stats IS
'Returns aggregate stats for filtered companies. Now supports growth filter parameters.';
