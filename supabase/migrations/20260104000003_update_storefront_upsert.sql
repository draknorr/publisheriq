-- Add DLC sync capability to upsert_storefront_app function
-- This version adds p_dlc_appids parameter to sync DLC relationships

-- Drop the old function signature (without p_dlc_appids) to avoid overloading
DROP FUNCTION IF EXISTS upsert_storefront_app(INTEGER, TEXT, TEXT, BOOLEAN, DATE, TEXT, BOOLEAN, INTEGER, INTEGER, BOOLEAN, TEXT[], TEXT[]);
...
CREATE OR REPLACE FUNCTION upsert_storefront_app(
  ...
  p_developers TEXT[],
  p_publishers TEXT[],
  p_dlc_appids INTEGER[] DEFAULT NULL  -- NEW: Array of DLC app IDs
)
RETURNS VOID AS $$
DECLARE
  v_dev_name TEXT;
...
COMMENT ON FUNCTION upsert_storefront_app(INTEGER, TEXT, TEXT, BOOLEAN, DATE, TEXT, BOOLEAN, INTEGER, INTEGER, BOOLEAN, TEXT[], TEXT[], INTEGER[]) IS
  'Optimized single-call function to update app with storefront data, developers, publishers, DLC relationships, and sync status';
