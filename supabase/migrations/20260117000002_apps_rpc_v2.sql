-- =============================================================================
-- Migration: Apps Page RPC v2 - Optimized with Materialized View
--
-- Performance improvements:
--   1. Uses app_filter_data materialized view for content filtering
--   2. Replaces EXISTS subqueries with array containment operators (&&, @>)
--   3. Pre-computed publisher/developer data eliminates LATERAL JOINs
--
-- Expected query time: 50-200ms (down from 5-30s+ timeouts)
-- =============================================================================

-- ============================================================================
-- Replace get_apps_with_filters with optimized version
-- ============================================================================
...
COMMENT ON FUNCTION get_apps_with_filters IS
  'Optimized Games page query using app_filter_data materialized view. Uses array containment operators instead of EXISTS subqueries. Expected query time: 50-200ms.';


-- ============================================================================
-- Update get_apps_aggregate_stats to use materialized view
-- ============================================================================
...
COMMENT ON FUNCTION get_apps_aggregate_stats IS
  'Optimized aggregate stats using app_filter_data materialized view for content filtering.';


-- ============================================================================
-- Update get_apps_filter_option_counts to use materialized view
-- ============================================================================
...
BEGIN
  IF p_filter_type = 'genre' THEN
    ...
    SELECT
      ...
      COUNT(DISTINCT afd.appid)::BIGINT AS app_count
    FROM steam_genres sg
    JOIN app_filter_data afd ON sg.genre_id = ANY(afd.genre_ids)
    JOIN apps a ON a.appid = afd.appid
    LEFT JOIN latest_daily_metrics ldm ON ldm.appid = a.appid
    WHERE a.is_released = TRUE AND a.is_delisted = FALSE
      AND (p_type = 'all' OR a.type::TEXT = p_type)
      AND (p_min_ccu IS NULL OR ldm.ccu_peak >= p_min_ccu)
  ...
  ELSIF p_filter_type = 'tag' THEN
    ...
    SELECT
      ...
      COUNT(DISTINCT afd.appid)::BIGINT AS app_count
    FROM steam_tags st
    JOIN app_filter_data afd ON st.tag_id = ANY(afd.tag_ids)
    JOIN apps a ON a.appid = afd.appid
    LEFT JOIN latest_daily_metrics ldm ON ldm.appid = a.appid
    WHERE a.is_released = TRUE AND a.is_delisted = FALSE
      AND (p_type = 'all' OR a.type::TEXT = p_type)
  ...
  ELSIF p_filter_type = 'category' THEN
    ...
    JOIN app_filter_data afd ON sc.category_id = ANY(afd.category_ids)
    JOIN apps a ON a.appid = afd.appid
    LEFT JOIN latest_daily_metrics ldm ON ldm.appid = a.appid
    WHERE a.is_released = TRUE AND a.is_delisted = FALSE
      AND (p_type = 'all' OR a.type::TEXT = p_type)
  ...
  ELSIF p_filter_type = 'steam_deck' THEN
    RETURN QUERY
    SELECT
      ROW_NUMBER() OVER (ORDER BY afd.steam_deck_category)::INT AS option_id,
      afd.steam_deck_category::TEXT AS option_name,
      COUNT(DISTINCT afd.appid)::BIGINT AS app_count
    ...
    GROUP BY afd.steam_deck_category
...
COMMENT ON FUNCTION get_apps_filter_option_counts IS
  'Filter option counts using app_filter_data materialized view for optimized content filtering.';
