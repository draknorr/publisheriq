-- =============================================================================
-- Migration: Add CCU Growth Columns to ccu_tier_assignments
-- Purpose: Pre-compute 7-day and 30-day CCU growth for Apps page
-- =============================================================================

-- Add growth percentage columns
ALTER TABLE ccu_tier_assignments
ADD COLUMN IF NOT EXISTS ccu_growth_7d_percent DECIMAL,
ADD COLUMN IF NOT EXISTS ccu_growth_30d_percent DECIMAL;

COMMENT ON COLUMN ccu_tier_assignments.ccu_growth_7d_percent IS '7-day CCU growth: ((last 7 days avg - prior 7 days avg) / prior avg) * 100';
COMMENT ON COLUMN ccu_tier_assignments.ccu_growth_30d_percent IS '30-day CCU growth: ((last 7 days avg - 30-day baseline avg) / baseline avg) * 100';

-- Add indexes for sorting and filtering by growth
CREATE INDEX IF NOT EXISTS idx_ccu_tier_assignments_growth_7d
  ON ccu_tier_assignments (ccu_growth_7d_percent DESC NULLS LAST);

CREATE INDEX IF NOT EXISTS idx_ccu_tier_assignments_growth_30d
  ON ccu_tier_assignments (ccu_growth_30d_percent DESC NULLS LAST);

-- =============================================================================
-- Updated Function: recalculate_ccu_tiers()
-- Now also computes CCU growth percentages
-- =============================================================================
...
BEGIN
  ...
  -- Step 5: Calculate CCU growth percentages
  -- 7-day growth: (last 7 days avg - prior 7 days avg) / prior avg * 100
  -- 30-day growth: (last 7 days avg - 30-day baseline avg) / baseline avg * 100
  CREATE TEMP TABLE ccu_growth ON COMMIT DROP AS
  SELECT
    appid,
    -- 7-day growth: compare last 7 days to prior 7 days
    CASE
      WHEN prior_7d_avg IS NULL OR prior_7d_avg = 0 THEN NULL
      ELSE ROUND(((last_7d_avg - prior_7d_avg) / prior_7d_avg) * 100, 2)
    END AS growth_7d_pct,
    -- 30-day growth: compare last 7 days to full 30-day baseline
    CASE
      WHEN baseline_30d_avg IS NULL OR baseline_30d_avg = 0 THEN NULL
      ELSE ROUND(((last_7d_avg - baseline_30d_avg) / baseline_30d_avg) * 100, 2)
    END AS growth_30d_pct
  FROM (
    SELECT
      appid,
      AVG(player_count) FILTER (WHERE snapshot_time > NOW() - INTERVAL '7 days') AS last_7d_avg,
      AVG(player_count) FILTER (WHERE snapshot_time > NOW() - INTERVAL '14 days' AND snapshot_time <= NOW() - INTERVAL '7 days') AS prior_7d_avg,
...
COMMENT ON FUNCTION recalculate_ccu_tiers() IS 'Recalculates tier assignments and CCU growth percentages. Tier 1 = top 500 by 7-day peak CCU, Tier 2 = 1000 newest releases not in Tier 1, Tier 3 = all others. Growth calculated from ccu_snapshots.';
