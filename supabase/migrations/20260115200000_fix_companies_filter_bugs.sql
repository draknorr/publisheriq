-- Migration: Fix companies filter bugs discovered during testing
-- Date: 2026-01-14
--
-- Fixes:
--   BUG #1: Slow path SQL error - add column aliases to base_companies CTE
--   BUG #3: Aggregate stats missing parameters - add genre_mode, max_* params
--
-- Note: Platform filtering (BUG #2) requires adding platform_array to materialized views
--       which is deferred to a future migration to avoid view recreation downtime.

-- ============================================================================
-- FIX BUG #1 & Add max_* filters to get_companies_with_filters
-- ============================================================================
...
BEGIN
  ...
  IF NOT v_needs_growth AND NOT v_needs_relationship THEN
    ...
    filtered AS (
      ...
      WHERE
        ...
        -- Platform filter placeholder - will be implemented when platform_array is added to views
    ),
    sorted_paginated AS (
      SELECT f.*
      FROM filtered f
  ...
  -- Slow path: Growth/relationship filters (BUG #1 FIX: Added column aliases)
  ELSE
    RETURN QUERY
    WITH base_companies AS (
      -- Publishers (with proper column aliases)
      SELECT
        p.id AS id,
        p.name AS name,
        'publisher'::TEXT AS type,
      ...
      -- Developers (BUG #1 FIX: Added column aliases - this was the broken part)
      SELECT
        d.id AS id,
        d.name AS name,
        'developer'::TEXT AS type,
...
'Unified RPC for /companies page. Fixed slow path column aliases (BUG #1).
Platform filtering placeholder - requires platform_array in materialized views.';


-- ============================================================================
-- FIX BUG #3: Add missing parameters to get_companies_aggregate_stats
-- ============================================================================

DROP FUNCTION IF EXISTS get_companies_aggregate_stats(
  TEXT, TEXT, INT, INT, BIGINT, INT, BIGINT, BIGINT, INT, INT, TEXT, INT[], INT[], INT[], TEXT
);
...
'Returns aggregate stats for filtered companies. Fixed: Added genre_mode and all max_* filters (BUG #3).';
