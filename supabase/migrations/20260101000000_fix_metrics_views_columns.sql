-- Migration: Fix materialized view columns to match Cube.dev models
-- Problem: The Cube models reference columns that don't exist in the views
-- This recreates both views with the correct column names

-- =============================================
-- FIX: Publisher Metrics View
-- =============================================
...
SELECT
  pa.publisher_id,
  p.name AS publisher_name,  -- Cube expects: publisherName
  p.game_count,              -- Cube expects: gameCount
  ...
  CASE
    ...
    ELSE NULL
  END AS avg_review_score,  -- Cube expects: avgReviewScore

  -- Revenue estimate in cents (Cube expects: revenueEstimateCents)
  COALESCE(SUM(
    ((COALESCE(pa.owners_min, 0) + COALESCE(pa.owners_max, 0)) / 2.0) *
    COALESCE(pa.current_price_cents, 0)
  ), 0)::BIGINT AS revenue_estimate_cents,

  -- Trending as boolean (Cube expects: isTrending)
...
COMMENT ON MATERIALIZED VIEW publisher_metrics IS
  'Pre-computed aggregate metrics for publishers with columns matching Cube.dev model. Refresh with: REFRESH MATERIALIZED VIEW CONCURRENTLY publisher_metrics';


-- =============================================
-- FIX: Developer Metrics View
-- =============================================
...
SELECT
  da.developer_id,
  d.name AS developer_name,  -- Cube expects: developerName
  d.game_count,              -- Cube expects: gameCount
...
COMMENT ON MATERIALIZED VIEW developer_metrics IS
  'Pre-computed aggregate metrics for developers with columns matching Cube.dev model. Refresh with: REFRESH MATERIALIZED VIEW CONCURRENTLY developer_metrics';
