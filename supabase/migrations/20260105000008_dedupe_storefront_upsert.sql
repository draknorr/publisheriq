-- Migration: Deduplicate developer/publisher arrays in upsert_storefront_app
--
-- Fixes error: "ON CONFLICT DO UPDATE command cannot affect row a second time"
-- This occurs when Steam returns duplicate developer/publisher names in the same array.
-- Adding DISTINCT to the valid_names CTE prevents duplicate inserts.
...
COMMENT ON FUNCTION upsert_storefront_app(INTEGER, TEXT, TEXT, BOOLEAN, DATE, TEXT, BOOLEAN, INTEGER, INTEGER, BOOLEAN, TEXT[], TEXT[], INTEGER[], INTEGER) IS
  'Single-call upsert with DISTINCT to handle duplicate dev/pub names from Steam';
