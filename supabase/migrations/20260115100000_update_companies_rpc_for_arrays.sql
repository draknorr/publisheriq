-- Migration: Update get_companies_with_filters to use pre-computed content arrays
-- Replaces slow EXISTS subqueries with fast GIN array containment queries
--
-- Performance improvement:
--   Tags: 7.7s -> <200ms (using && operator on tag_ids array)
--   Genres: 5.5s -> <200ms (using && operator on genre_ids array)
--   Categories: ~2s -> <100ms (using && operator on category_ids array)
--   Steam Deck: 1.4s -> <100ms (using simple equality on best_steam_deck_category)
...
CREATE OR REPLACE FUNCTION get_companies_with_filters(
  ...
  -- Content filters (now using pre-computed arrays)
  ...
  IF NOT v_needs_growth AND NOT v_needs_relationship THEN
    ...
    WITH base_companies AS (
      ...
      SELECT
        ...
        -- NEW: Pre-computed content arrays
        pm.genre_ids,
        pm.tag_ids,
        pm.category_ids,
    ...
    filtered AS (
      ...
      WHERE
        ...
        -- OPTIMIZED: Use pre-computed arrays with GIN index (was EXISTS subquery)
        AND (p_genres IS NULL OR (
          CASE p_genre_mode
            WHEN 'all' THEN bc.genre_ids @> p_genres  -- has ALL specified genres
            ELSE bc.genre_ids && p_genres             -- has ANY specified genre
          END
        ))
        -- OPTIMIZED: Use pre-computed tag array (was 7.7s EXISTS, now <200ms)
        AND (p_tags IS NULL OR bc.tag_ids && p_tags)
        -- OPTIMIZED: Use pre-computed category array (was 2s EXISTS, now <100ms)
        AND (p_categories IS NULL OR bc.category_ids && p_categories)
        -- OPTIMIZED: Use pre-computed steam deck category (was 1.4s EXISTS, now <100ms)
        AND (p_steam_deck IS NULL OR (
          CASE p_steam_deck
            ...
            ELSE bc.best_steam_deck_category IS NOT NULL  -- 'any' = has any steam deck status
          END
        ))
    ),
    sorted_paginated AS (
    ...
    filtered AS (
      ...
      WHERE (p_search IS NULL OR bc.name ILIKE '%' || p_search || '%')
        ...
        AND (p_status IS NULL
             OR (p_status = 'active' AND bc.games_released_last_year > 0)
             OR (p_status = 'dormant' AND bc.games_released_last_year = 0))
        -- OPTIMIZED: Use pre-computed arrays (was slow EXISTS subqueries)
        AND (p_genres IS NULL OR (
          CASE p_genre_mode
...
OPTIMIZED: Content filters (genres, tags, categories, steam_deck) now use pre-computed
arrays with GIN indexes instead of EXISTS subqueries. Performance: 7.7s -> <200ms.
Two code paths:
- Fast path: when no growth/relationship filters are applied
- Slow path: when growth filtering or sorting is needed';


-- ============================================================================
-- Also update get_companies_aggregate_stats to use pre-computed arrays
-- ============================================================================
...
BEGIN
  ...
  filtered AS (
    ...
    WHERE (p_search IS NULL OR b.name ILIKE '%' || p_search || '%')
      ...
      -- OPTIMIZED: Use pre-computed arrays instead of EXISTS subqueries
      AND (p_genres IS NULL OR b.genre_ids && p_genres)
...
'Returns aggregate stats for filtered companies. OPTIMIZED: Uses pre-computed content arrays.'
